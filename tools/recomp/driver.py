...
def recompile(exe_path, catalog_path, outdir, addrs=None, split=400,
    ...
    # Clear the last run's translation units first.
    #
    # The count of these follows the number of functions, so a round that lifts
    # FEWER of them writes fewer files and leaves the tail of the previous run
    # on disk - and a game project globs the directory, so the stale ones are
    # still compiled. Every function in them is then defined twice and the link
    # fails with several thousand LNK2005s that name a symbol and nothing about
    # the cause.
    #
    # It happened the first time a catalog fix REMOVED entries: 31,095
    # functions became 30,536, file 0077 was never rewritten, and the build
    # broke on a file two hours older than the rest of the directory.
    #
    # Only our own generated names, never the whole directory - a game project
    # keeps its own sources next door.
    if addrs is None:
        for stale in glob.glob(os.path.join(outdir, "recomp_funcs_[0-9]*.c")):
            os.remove(stale)
    ...
    emit_headers(outdir, done, iat, purge_table(exe_path, iat), exe_path)
...
def image_fingerprint(path):
    """A cheap, stable hash of the executable this build was lifted from.

    Lifted code and the mapped image are two halves of one program: the C is
    compiled from one file's bytes and reads its constants out of another
    file's. They have to be the same file, and nothing checked it - the runtime
    took whatever path it was handed.

    Mario Kart's tree ships two, the same size and 33 bytes apart, and the
    difference is AT THE ENTRY POINT: one jumps to a stub that loads the
    cabinet's I/O emulator, the other does not. Lifting the patched one and
    running the clean one meant executing that stub while reading the string it
    passes out of an image where those bytes are zero - so the game asked for a
    DLL named "". It boots either way, which is exactly the problem.

    FNV-1a over the whole file. Not a cryptographic answer; the question is
    "is this the same file", which it answers.
    """
...
def emit_headers(outdir, done, iat, purge=None, exe_path=None):
    """The three generated headers the runtime compiles against."""
    purge = purge or {}
    os.makedirs(outdir, exist_ok=True)
    if exe_path:
        with open(os.path.join(outdir, "recomp_image.h"), "w") as f:
            f.write('/* AUTO-GENERATED by systemes3recomp - the executable this\n'
                    '   build was lifted from. guest_load() checks it. */\n'
                    '#define ES3_IMAGE_NAME "%s"\n'
                    '#define ES3_IMAGE_SIZE %uu\n'
                    '#define ES3_IMAGE_HASH 0x%016XULL\n'
                    % (os.path.basename(exe_path),
                       os.path.getsize(exe_path), image_fingerprint(exe_path)))
